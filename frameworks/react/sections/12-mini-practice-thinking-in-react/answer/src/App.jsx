...
export default function App(){
 ...
 const list=useMemo(()=>{
   const byName=products.filter(p=>p.name.toLowerCase().includes(q.toLowerCase()));
   return byName.filter(p=>onlyStock?p.inStock:true);
 },[q,onlyStock]);
 return <div className="container"><h1>12 Thinking in React（答案）</h1><div className="row"><input value={q} onChange={e=>setQ(e.target.value)} placeholder="搜索商品"/><label><input type="checkbox" checked={onlyStock} onChange={e=>setOnlyStock(e.target.checked)}/>仅看有货</label></div>{list.map(p=><div className="card" key={p.id}>{p.name} - ￥{p.price} {p.inStock?'✅':'❌'}</div>)}</div>;
}
